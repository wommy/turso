...
Start a Model Context Protocol server over HTTP instead of the interactive shell, listening at the given address. Requests are POSTed as JSON-RPC to `/mcp`.
...
<Warning>
Serves both the `2026-07-28` revision and the older `initialize` handshake. Connections are handled one at a time - run `.manual mcp` in the shell for details and limitations.
</Warning>
